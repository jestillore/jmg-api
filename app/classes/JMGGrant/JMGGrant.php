 ...
 * JMG grant class
 */
class JMGGrant implements GrantTypeInterface {
    ...
    /**
     * Callback to authenticate the secret field
     * @var function
     */
    ...
    private $secretFieldName = 'secreto';
    ...
    /**
     * Set the callback to verify a secret field
    ...
    public function completeFlow($inputParams = null)
    {
        ...
        $authParams = $this->authServer->getParam(array('client_id', 'client_secret', $this->secretFieldName), 'post', $inputParams);
        ...
        if (is_null($authParams[$this->secretFieldName])) {
            throw new Exception\ClientException(sprintf($this->authServer->getExceptionMessage('invalid_request'), $this->secretFieldName), 0);
        }

        // Check if secret field is correct
        $userId = call_user_func($this->getVerifyCredentialsCallback(), $authParams[$this->secretFieldName]);
